...
export const createSavingsSchema = plan;
export const updateSavingsSchema = plan;
export const depositSavingsSchema = entry;
export const withdrawSavingsSchema = entry;
export const savingsQuerySchema = z.object({ page: z.coerce.number().int().min(1).default(1), limit: z.coerce.number().int().min(1).max(100).default(20), search: z.string().trim().max(100).optional(), currency: z.enum(['USD', 'SOS']).optional(), status: z.enum(['ACTIVE', 'NEAR_TARGET', 'COMPLETED', 'OVERDUE']).optional(), isActive: z.coerce.boolean().optional() });
export const savingsHistoryQuerySchema = z.object({ page: z.coerce.number().int().min(1).default(1), limit: z.coerce.number().int().min(1).max(100).default(20) });
export type SavingsInput = z.infer<typeof plan>;
export type SavingsEntryInput = z.infer<typeof entry>;
